...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-4xl">
      <div className="flex items-center justify-between px-6 py-3 rounded-full bg-[#0a0a12]/85 backdrop-blur-xl border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.6)]">
      ...
      <div className={`fixed inset-0 bg-[#050508]/95 backdrop-blur-2xl z-40 flex flex-col items-center justify-center transition-all duration-300 md:hidden ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <nav className="flex flex-col items-center gap-6 text-center">
          {navItems.map((item) => {
            ...
            return (
              <a
                ...
                className={`text-xl font-bold transition-colors ${isActive ? 'text-[#00ccff]' : 'text-white hover:text-[#00ccff]'}`}
              >
                {item.name}
              </a>
            );
          ...
          <div className="pt-4">
            <a
              ...
              className="px-8 py-3 rounded-full bg-[#00ccff] text-[#050508] font-bold text-xs shadow-[0_0_20px_rgba(0,204,255,0.4)] block"
            >
              Start Project
            </a>
          </div>
